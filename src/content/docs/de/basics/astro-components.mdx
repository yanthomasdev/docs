...
i18nReady: true
---
if this doesnt work god is a clown
...
Das Wichtigste an Astro-Komponenten ist, dass sie **nicht auf dem Client rendern**. Sie werden entweder zur Build-Zeit oder auf Anfrage mittels [server-seitigem Rendering (SSR)](/de/guides/server-side-rendering/) zu HTML gerendert. Du kannst JavaScript-Code innerhalb deines Komponenten-Frontmatters einbinden, und all dieser wird von der endgültigen Seite entfernt, die an deine Nutzerinnen und Nutzer gesendet wird. Das Ergebnis ist eine schnellere Seite, welche standardmäßig kein JavaScript enthält.

Wenn deine Astro-Komponente clientseitige Interaktivität benötigt, kannst du [Standard-HTML-`<script>`-Tags](/de/guides/client-side-scripts/) oder [UI-Framework-Komponenten](/de/guides/framework-components/#interaktive-komponenten-hydratisieren) hinzufügen.
